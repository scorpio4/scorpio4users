...
**/
		...
		var grid = my.UX.Widget({ "this": "urn:example:ux:grid", type: "urn:scorpio4:ux:Grid", label: "My Fact Sheet", collection: models_grid,
		    selectable: false, paging: false,
			factualizer: _default_factualizer,
			meta: models_grid_columns
		});

		var toolbar = my.UX.Widget({ "this": "urn:example:ux:toolbar", type: "urn:scorpio4:ux:Toolbar", label: "Toolbar", collection: toolbar_menu });

		var filter = my.UX.Widget({ "this": "urn:example:ux:filter", type: "urn:scorpio4:ux:FacetFilter", label: "Filter", meta: models_grid_columns })

		var list = my.UX.Widget({ "this": "urn:example:ux:list", type: "urn:scorpio4:ux:Tree", label: "List", collection: models_grid_columns,
		    selectable: true,
		    factualizer: _default_factualizer, follow: "urn:example:model:concepts:child"
		});

		var form = my.UX.Panel({ "this": "urn:example:ux:form", type: "urn:scorpio4:ux:Form", label: "My Form",
		    selectable: false,
			factualizer: _default_factualizer, meta: models_grid_columns,
		    dialog: false, modal: false
		});

		var navbar = my.UX.Widget({ "this": "urn:example:ux:navbar", type: "urn:scorpio4:ux:NavBar", label: "Demo", collection: models_grid,
		    selectable: false,
			factualizer: _default_factualizer,
			el: $("#my_navbar")
		}).render();
		...
		var layout = my.UX.Widget({ "this": "urn:example:ux:layout", type: "urn:scorpio4:ux:Layout", label: "Layout", el: $("#my_scorpio4"),
		    views: { "north": toolbar, "west": list, "center": grid, "east": filter }
		});

		var listViews = my.UX.Panel({ "this": "urn:example:ux:views", type: "urn:scorpio4:ux:List", label: "List of views", collection: models_list_views, dialog: true });
        listViews.show();
