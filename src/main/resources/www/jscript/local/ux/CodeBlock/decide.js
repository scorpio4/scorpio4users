...
Blockly.Language.iq_if = {
  // If/elseif/else conditionion.
  category: "Decisions",
  ...
  init: function() {
    ...
    this.setMutator(new Blockly.Mutator(['iq_if_elseif',
                                         'iq_if_else']));
  ...
  domToMutation: function(xmlElement) {
    ...
    for (var x = 1; x <= this.elseifCount_; x++) {
      this.appendValueInput('IF' + x)
          .setCheck(Boolean)
          .appendTitle("else if"); // Blockly.LANG_CONTROLS_IF_MSG_ELSEIF
  ...
  decompose: function(workspace) {
    var containerBlock = new Blockly.Block(workspace, 'iq_if_if');
    containerBlock.initSvg();
    var connection = containerBlock.getInput('STACK').connection;
    for (var x = 1; x <= this.elseifCount_; x++) {
      var elseifBlock = new Blockly.Block(workspace, 'iq_if_elseif');
      elseifBlock.initSvg();
      connection.connect(elseifBlock.previousConnection);
      connection = elseifBlock.nextConnection;
    }
    if (this.elseCount_) {
      var elseBlock = new Blockly.Block(workspace, 'iq_if_else');
      elseBlock.initSvg();
