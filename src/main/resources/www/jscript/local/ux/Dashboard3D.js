(function($, my) {
	...
	my.scorpio4.UX.View["urn:scorpio4:ux:Dashboard3D"] = function(options) {
		...
		var ViewStep = Backbone.Marionette.ItemView.extend( {
			...
			templateHelpers: my.scorpio4.UX.mixin.Common.templateHelpers,
...
//				if (!type) throw "urn:scorpio4:ux:oops:missing-view-facet";
				var itemView = options.viewable[type] || ViewStep;
				DEBUG && console.log("3D View Type:", type, model, itemView )
//				if (!itemView) throw "urn:scorpio4:ux:oops:missing-view#"+type;
//				return itemView(viewOptions);
			...
			renderItemView: function(view, index) {
				...
				if (!that) throw "urn:scorpio4:ux:oops:missing-model-identity#"+that;

				var uid = my.scorpio4.UX.uid( that );
			...
			positionItemView: function(view, coords) {
				if (!view) throw "urn:scorpio4:ux:oops:missing-view#";
				if (!view.model) throw "urn:scorpio4:ux:oops:missing-view-model#";
				if (!coords) throw "urn:scorpio4:ux:oops:missing-coords#";
...
//				this.triggerMethod("activate" );
			...
			_select: my.scorpio4.UX.mixin.Selectable.select,
			selectItem: function(model) {
				var uid = my.scorpio4.UX.uid( model.id );
