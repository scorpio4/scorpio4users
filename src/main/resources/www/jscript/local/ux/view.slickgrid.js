...
//      this.grid_options = modelEtc.options;
    ...
    this.visible = ("visible" in modelEtc)?(modelEtc.visible?true:false):true; // [LC] default to visible
  },
  ...
  render: function() {
    ...
    // Selection Handling [LC]
	if (!this.state.get('selection') || this.state.get('selection')=="row") {
		this.grid.setSelectionModel(new Slick.RowSelectionModel());
		this.grid.onSelectedRowsChanged.subscribe(function(e,args) {
			if (self.selection) {
				self.trigger("deselect", self.selection);
			}
			var selectedRows = [];
			for(var i=0;i<args.rows.length;i++) {
				var data = args.grid.getDataItem(args.rows[i]);
				if (data) selectedRows.push(data);
			}
			if (!self.selection) {
			...
			self.selection.add(selectedRows);
			if (selectedRows&&selectedRows.length>0) {
				self.trigger("select", self.selection);
			}
		})
	}
	// end [LC]
