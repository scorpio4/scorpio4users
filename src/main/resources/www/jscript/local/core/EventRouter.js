(function($, my){
	my.RoutePlanner = function(options, events) {
		if (!options) throw "urn:scorpio4:ux:oops:no-options";

		this.destination = options.destination || options.to || events;
		if (!this.destination) throw "urn:scorpio4:ux:oops:no-destination";
		...
		if (!this.base) {
			...
			throw "urn:scorpio4:ux:oops:no-base-uri";
		}
		...
		this.plan = function(route, meta) {
			if (!route.from) throw "urn:scorpio4:ux:oops:missing-from";
			if (!route.to) throw "urn:scorpio4:ux:oops:missing-to";
			var self = this;
...
//							console.log("fn: %o %o", route, route.then)
			...
			if (typeof(route.to) == "object") {
				// complex route
				if (!route.to.to) throw "urn:scorpio4:ux:oops:missing-next-to";
...
//				self.plan(route.to);
				...
				throw "urn:scorpio4:ux:oops:unknown-destination: "+route.to;
			}
		}
		return self;
	}
