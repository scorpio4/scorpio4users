(function($, my) {
	...
	_.extend(my.scorpio4.fact.finder, { Local: {} });

	// Implement Backbone.Collection factory
	my.scorpio4.fact.finder.Local.Models = function(options, bindings) {
...
//			var FactCollection = Backbone.Collection.extend( { model: factModel }, options );
		...
		models.sync = my.scorpio4.fact.finder.Local.Sync(options)
...
my.scorpio4.DEBUG && console.debug("[Local]%s Models Reloaded: %o %o %o %s %o %o", models.length, models.$from, that, models, (bindings?"not bound":"bindings"), bindings, result)
	...
	my.scorpio4.fact.finder.Local.Model = function(options, model) {
		...
		var models = my.scorpio4.fact.finder.Local.Models(options,bindings);
console.debug("[Local]Build Model:", model, models, options, bindings );
	...
	my.scorpio4.fact.finder.Local.Sync = function(options, bindings) {
		return function(method, models, actions) {
my.scorpio4.fact.DEBUG && console.debug("[Local]Local Finder:", method, models, actions);
			var options = { "this": my.scorpio4.fact.urn("urn:scorpio4:dummy"), "from": "urn:scorpio4:dummy" }
			var models = my.scorpio4.fact.finder.Local.Models(options);
			return models;
		}
	}

	// Implement 'read' method of a Backbone.Collection.sync
	my.scorpio4.fact.finder.Local.read = function(root, query, actions, bindings) {
		return root;
	}
