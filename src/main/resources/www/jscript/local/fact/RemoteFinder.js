(function($, my) {
	...
	_.extend(my.scorpio4.fact.finder, { Remote: {} });

	// Implement Backbone.Collection factory
	my.scorpio4.fact.finder.Remote.Models = function(options, bindings) {
...
//			var FactCollection = Backbone.Collection.extend( { model: factModel }, options );
		...
		models.sync = my.scorpio4.fact.finder.Remote.Sync(options)
...
my.scorpio4.DEBUG && console.debug("[Remote]%s Models Reloaded: %o %o %o %s %o %o", models.length, models.$from, that, models, (bindings?"not bound":"bindings"), bindings, result)
	...
	my.scorpio4.fact.finder.Remote.Model = function(options, model) {
...
console.debug("[Remote] Fetch:", identity, bindings );
	...
	my.scorpio4.fact.finder.Remote.Sync = function(options, bindings) {
		return function(method, models, actions) {
my.scorpio4.fact.DEBUG && console.debug("[Remote] Sync:", method, models, actions);
			my.scorpio4.fact.finder.Remote[method](models, options, actions, bindings);
			return false;
		}
	}

	// Implement 'read' method of a Backbone.Collection.sync
	my.scorpio4.fact.finder.Remote.read = function(root, query, actions, bindings) {
my.scorpio4.fact.DEBUG && console.debug("[Remote] Read:", root, query, actions, bindings);
        ...
        var promise = $.ajax( { url: "/rest/crud/"+query.this , data: query, datatype: "json", async: true }).done( function(reply) {
            ...
            root.add(models);
            my.scorpio4.DEBUG && console.debug( "[Remote] READ DONE: ", query, reply, models, root);
