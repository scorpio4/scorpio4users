...
_.extend(my.scorpio4.fact.finder, { Core: {
    DEBUG: true,
...
my.scorpio4.fact.finder.Core.DEBUG && console.log("[Core] fetch URL:", this, that)
            return "dummy";
		}
		// create a Paged Collection
		var FactModel = my.scorpio4.fact.factory.FactModel.extend({
		    url: bb_default_url
		})
//			var FactCollection = Backbone.Collection.extend( { model: factModel }, options );
		...
		models.sync = my.scorpio4.fact.finder.Core.Sync(options)

		// locate $from (RDF databank)
		var that = options["this"];
		var from = options["from"];
		if (from) {
			models.$from = my.scorpio4.fact.Finder(from);
...
my.scorpio4.fact.finder.Core.DEBUG && console.debug("[Core] %s onReload: %o %o %o %s %o %o", models.models.length, models.$from, that, models, (bindings?"bound":"UN-bound"), bindings, result)
...
my.scorpio4.fact.finder.Core.DEBUG && console.debug("[Core] Pre-fetch: ", models, bindings)
				models.fetch();
			}
		}
...
my.scorpio4.fact.finder.Core.DEBUG && console.debug("[Core] Model: ", query, model);

		// dynamic configuration
		if ( _.isString(query) ) query = my.scorpio4.fact.asq[query]; // retrieve query by reference
		if ( !_.isObject(query) ) throw "urn:scorpio4:fact:finder:oops:invalid-query";
		...
		if (query.refocus) {
		    ...
		    throw "urn:scorpio4:fact:finder:oops:invalid-refocus";
		}
...
my.scorpio4.fact.finder.Core.DEBUG && console.debug("[Core] Initialize Model:", model, models, query, bindings );
	...
	Sync: function(options, bindings) {
		...
		return function(method, models, actions) {
			...
			if (!crud) throw "urn:scorpio4:fact:finder:oops:missing-method#"+method;
	...
	read: function(root, query, actions, bindings) {
		...
		if (!root) throw "urn:scorpio4:ux:oops:missing-root-model";
		if (!root.$from) throw "urn:scorpio4:ux:oops:missing-from-finder";

		root.trigger("before:query");
		var $core = $.rdf({databank: root.$from});
		var idAttribute = my.scorpio4.idAttribute;

my.scorpio4.fact.finder.Core.DEBUG && console.debug("Reading %s -> %s: $:%o r:%o q:%o a:%o b:%o", (query.follow?"TREE":"TABLE"), query["this"], $core,  root, query, actions, bindings)

		var asq = new my.scorpio4.ASQ.Core();

		var fn_root_query = asq.build(query);
		var results = fn_root_query( $core, bindings );
		root.reset();
		var models = my.scorpio4.fact.finder.Core.toArray(results);
// my.scorpio4.fact.finder.Core.DEBUG && console.debug("Root Results", root, results, models)
		...
		var follow_query = _.extend( { "this": query[my.scorpio4.idAttribute] , where: [] } );
		follow_query.where.push.apply( follow_query.where, query.follow.where );
		var fn_follow = asq.build(follow_query);
my.scorpio4.fact.finder.Core.DEBUG && console.debug("Tree Follow: %o %o -> %o %o", childAttribute, follow_query, root, models);
...
// my.scorpio4.fact.finder.Core.DEBUG && console.debug("other: %o %o", other_id, other);
		...
		root.model = TreeNode;
my.scorpio4.fact.finder.Core.DEBUG && console.debug("Tree Add: ", root, models);
/*
...
*/
		...
		root.trigger("query");
		root.trigger("change", bindings);
my.scorpio4.fact.finder.Core.DEBUG && console.debug("Tree Changed:", root);
