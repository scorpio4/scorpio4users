(function($, my){
	...
	my.scorpio4.ASQ.Common = {

        build: function(spec) {
            if (_.isFunction(spec)) return spec;
            var self = this;
            this.base = spec["this"] || this.base || "urn:scorpio4:ux:query#";
            if (!this.base) throw "urn:scorpio4:asq:oops:no-base-uri";
            if (!spec.where) throw "urn:scorpio4:asq:oops:missing-where";
...
self.DEBUG && console.debug("Run asq: ", spec["this"], spec, from, model)
        ...
        describe: function(query, from) {
            if (!query.where) throw "urn:scorpio4:asq:oops:missing-where";
            this.base = query["this"] || this.base || "urn:scorpio4:asq:query:";
            var self = this;
...
self.DEBUG && console.debug("==> %o %o", value.query, this);
        ...
        execute: function(spec, from, model) {
            var self = this;
            if (!from) throw "urn:scorpio4:asq:oops:no-from";
            if (!spec.where) throw "urn:scorpio4:asq:oops:no-where-clause";
...
self.DEBUG && console.debug("Invalid %s Where: %o", type, spec)
                throw "urn:scorpio4:asq:oops:invalid-where-clause";
            }
        },

        where: function(spec, from, model) {
            throw "urn:scorpio4:asq:oops:not-implemented";
        },
