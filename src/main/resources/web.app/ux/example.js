$(document).ready(function() {
	...
	var my = factcore.boot({"this": "urn:example:app:hello"});

	my.on("booted", function() {

		var query1 = {
			"this": 	"urn:factcore:ux:query:model:Example#",
			"where": [
				{
					"this"	: "<urn:factcore:ux:query:model:Example#this>",
					"has"	: "a",
...
//		grid.render();
		...
		my.UX["urn:factcore:ux:TreeView"] = Backbone.Marionette.CompositeView.extend({
			initialize: function(){
...
//				}
		...
		my.UX["urn:factcore:ux:Tree"] = Backbone.Marionette.CompositeView.extend({
			itemView: my.UX["urn:factcore:ux:TreeView"], template: "<ul class='node'></ul>",
			events: {
...
//				$("ul:first",this.$el).dcVerticalMegaMenu();
		...
		var treeView = new my.UX["urn:factcore:ux:Tree"]( { collection: treeModel });
//		treeView.render();
		treeView.on("select", function(focus) {
			console.log("Selected: %o %o", this, focus);
		})

		var workspace = my.UX["urn:factcore:ux:WorkSpace"]({
