$(document).ready(function() {
	...
	var application = "urn:factcore:application:predictsale:";
	var model_finder = application+"fact:finder:core";
	...
	var _default_factualizer = { "this": "urn:factcore:application:predictsale:factualizer", trigger: "hover", selector: "[about]", view: Marionette.ItemView.extend({ template: "<span>About: {{label}} - {{comment}}</span>" }) }
...
console.debug("Booted IQ: %o %o", factcore, my.core() );
		...
		var query_grid_columns =  my.fact.asq.register( {
			"this":	"urn:factcore:application:predictsale:model:concepts:columns:", lazy: false,
			"from":		model_finder, localize: true,
			"where": [
				{
					"this"	: "<urn:factcore:application:predictsale:ux:pipelineList>",
		...
		var query_list_views =  my.fact.asq.register( {
			"this":	"urn:factcore:application:predictsale:model:concepts:views:", lazy: false,
			"from":		model_finder, localize: true,
			"where": [
				{
					"this"	: "?this",
		...
		var query_grid =  my.fact.asq.register( {
			"this":	"urn:factcore:application:predictsale:model:concepts:", lazy: false,
			"from":		model_finder,
			"where": [
				{
					"this"	: "?this",
			...
			"follow": {
				"this":	"urn:factcore:application:predictsale:model:concepts:child",
				"where": [
					{
						"this"	: "?this",
						"has"	: "a",
...
**/
		...
		var toolbar_menu = new my.fact.factory.FactModels([
			{ "this": "urn:factcore:application:predictsale:ux:toolbar#create", label: "Create", comment: "Create entry", icon: "plus"},
			{ "this": "urn:factcore:application:predictsale:ux:toolbar#delete", label: "Delete", comment: "Delete entry", icon: "minus"},
			{ "this": "urn:factcore:application:predictsale:ux:toolbar#edit", label: "Edit", comment: "Edit entry", icon: "edit"}
		]);

        // Instantiate the UI

		var grid = my.UX.Widget({ "this": "urn:factcore:application:predictsale:ux:grid", type: "urn:factcore:ux:Grid", label: "My Fact Sheet", collection: models_grid,
		    selectable: false, paging: false,
			factualizer: _default_factualizer,
			meta: models_grid_columns
		});

		var toolbar = my.UX.Widget({ "this": "urn:factcore:application:predictsale:ux:toolbar", type: "urn:factcore:ux:Toolbar", label: "Toolbar", collection: toolbar_menu });

		var filter = my.UX.Widget({ "this": "urn:factcore:application:predictsale:ux:filter", type: "urn:factcore:ux:FacetFilter", label: "Filter", meta: models_grid_columns })

		var list = my.UX.Widget({ "this": "urn:factcore:application:predictsale:ux:list", type: "urn:factcore:ux:Tree", label: "List", collection: models_grid_columns,
		    selectable: true,
		    factualizer: _default_factualizer, follow: "urn:factcore:application:predictsale:model:concepts:child"
		});

		var form = my.UX.Panel({ "this": "urn:factcore:application:predictsale:ux:form", type: "urn:factcore:ux:Form", label: "My Form",
		    selectable: false,
			factualizer: _default_factualizer, meta: models_grid_columns,
		    dialog: false, modal: false
		});

		var navbar = my.UX.Widget({ "this": "urn:factcore:application:predictsale:ux:navbar", type: "urn:factcore:ux:NavBar", label: "Demo", collection: models_grid,
		    selectable: false,
			factualizer: _default_factualizer,
			el: $("#my_navbar")
		}).render();
		...
		var layout = my.UX.Widget({ "this": "urn:factcore:application:predictsale:ux:layout", type: "urn:factcore:ux:Layout", label: "Layout", el: $("#my_factcore"),
		    views: { "north": toolbar, "west": list, "center": grid, "east": filter }
		});

		var listViews = my.UX.Panel({ "this": "urn:factcore:application:predictsale:ux:views", type: "urn:factcore:ux:List", label: "List of views", collection: models_list_views, dialog: true });
        listViews.show();
