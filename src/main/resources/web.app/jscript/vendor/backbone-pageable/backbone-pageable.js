...
(function (factory) {
  ...
  function finiteInt (val, name) {
    if (!_.isNumber(val) || _.isNaN(val) || !_.isFinite(val) || ~~val !== val) {
      throw new TypeError("`" + name + "` must be a finite integer ("+val+")");
    }
    return val;
  }
    ...
    sync: function (method, model, options) {
      ...
      if (self.mode == "infinite") {
        ...
        options.success = function (resp, status, xhr) {
          ...
          // FIX: LC - check that currentPage is > firstPage
          if (newLinks.prev && currentPage>self.state.firstPage) links[currentPage - 1] = newLinks.prev;
