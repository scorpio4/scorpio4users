(function($, my) {
	...
	_.extend(my.factcore.fact.finder, { Dummy: {} });

	// Implement Backbone.Collection factory
	my.factcore.fact.finder.Dummy.Models = function(options, bindings) {
...
//			var FactCollection = Backbone.Collection.extend( { model: factModel }, options );
		...
		models.sync = my.factcore.fact.finder.Dummy.Sync(options)
...
my.factcore.DEBUG && console.debug("[Dummy]%s Models Reloaded: %o %o %o %s %o %o", models.length, models.$from, that, models, (bindings?"not bound":"bindings"), bindings, result)
	...
	my.factcore.fact.finder.Dummy.Model = function(options, model) {
		...
		var models = my.factcore.fact.finder.Dummy.Models(options,bindings);
console.debug("[Dummy]Build Model:", model, models, options, bindings );
	...
	my.factcore.fact.finder.Dummy.Sync = function(options, bindings) {
		return function(method, models, actions) {
my.factcore.fact.DEBUG && console.debug("[Dummy]Dummy Finder:", method, models, actions);
			var options = { "this": my.factcore.fact.urn("urn:factcore:dummy"), "from": "urn:factcore:dummy" }
			var models = my.factcore.fact.finder.Dummy.Models(options);
			return models;
		}
	}

	// Implement 'read' method of a Backbone.Collection.sync
	my.factcore.fact.finder.Dummy.read = function(root, query, actions, bindings) {
		return root;
	}
