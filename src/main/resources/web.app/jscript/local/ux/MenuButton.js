(function($, my) {
	...
	my.factcore.UX.View["urn:factcore:ux:MenuButton"] = function(options) {
...
//			template: "<a class='selectable' about='{{this}}' title='{{label}}'>{{label}}</a>"
		...
		var ButtonMenu = Backbone.Marionette.CompositeView.extend( _.extend({
			...
			selectItem: function(selection) {
				...
				if (!label) throw "urn:factcore:ux:oops:missing-selection-label";
	...
	my.factcore.UX.View["urn:factcore:ux:ButtonBar"] = function(options) {
		options = my.factcore.UX.checkOptions(options, ["this", "label", "collection"]);
		var DEBUG = true;
		...
		}, my.factcore.UX.mixin.Common, my.factcore.UX.mixin.Selectable ));
