(function($, my) {
	...
	my.factcore.UX.View["urn:factcore:ux:Tabs"] = function(options) {
		...
		var Tabs = Backbone.Marionette.CompositeView.extend({
			className: my.factcore.UX.stylize("ux_tab_panel tabbable", options),
...
DEBUG && console.log("Tab init", this, _options, this.collection);
			...
			add: function(view) {
				if (!view.model) throw "urn:factcore:ux:oops:missing-model";
				var _id = my.factcore.UX.uid(view.model.get("this")); // sanitize for HTML id
