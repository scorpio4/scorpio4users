(function($, my) {
	...
	my.factcore.UX.View["urn:factcore:ux:Form"] = function(options) {
...
DEBUG && console.log("Remodel: ", v, k, l);
		...
		if (_.isArray(options.meta)) schemaModel = my.factcore.fact.Models(options.meta);
		else if (options.meta instanceof Backbone.Collection) schemaModel = options.meta;
		else throw "urn:factcore:ux:oops:invalid-meta-data";
		...
		var Form = Backbone.Form.extend( _.extend({
			...
			initialize: function() {
				...
				if (!Form.templates.form) throw "urn:factcore:ux:oops:missing-form-template";
				var self = this;
				//Get the schema
...
DEBUG && console.log("Form Schema:", field.id, field, _schema)
					...
					throw "urn:factcore:ux:oops:missing-meta-data";
				})();
				...
				this.fields = {};
				_.extend(this, my.factcore.UX.mixin.Common);
