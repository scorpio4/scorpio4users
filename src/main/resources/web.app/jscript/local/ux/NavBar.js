(function($, my) {
	...
	my.factcore.UX.View["urn:factcore:ux:NavBar"] = function(options) {
		options = my.factcore.UX.checkOptions(options); // sanity check ('this', 'label')
...
DEBUG && console.log("MenuToggle", this, _options);
		...
		var NavBar = Backbone.Marionette.ItemView.extend({
			tagName: "div",
			className: my.factcore.UX.stylize("ux_navbar navbar",options),
...
DEBUG && console.log("On NavBar:", this, event, that);
			...
			onRender: function() {
    			...
    			var css = my.factcore.UX.stylize("ux_navbar navbar",options);
