(function($, my) {
	...
	my.factcore.UX.View["urn:factcore:ux:HTML"] = function(options) {
		...
		editor_options.toolbar = _.extend({
			"reference": function(locale, _options) {
				...
				return "<li><div class=''><a class='btn' data-factcore-event='reference' title='" + label + "'><i class='cog'></i>&nbsp;Reference</a></div></li>";
			},
			"viewer": function(locale, _options) {
				var label = (locale.reference && locale.reference.edit) || "Viewer";
				return "<li><div class=''><a class='btn' data-factcore-event='viewer' title='" + label + "'>Viewer</a></div></li>";
			},
/*
			"galleryx": function(locale, _options) {
				var label = (locale.reference && locale.reference.edit) || "Gallery";
				return "<li><div class='btn-group'><a class='btn' data-factcore-event='gallery' title='" + label + "'>Gallery</a></div></li>";
			},
*/
...
console.debug("Toolbar: Extra: %o %o", locale, _options);
				var html =
				"<li class='dropdown'>" +
					 "<a class='btn dropdown-toggle' data-factcore-event='dummy' data-toggle='dropdown' href='#' title='"+label+"'>" +
			...
			"thing": function(locale, _options) {
				...
				return "<li><div class='btn-group'><a class='btn' data-factcore-event='thing' title='" + label + "'>"+label+"</a></div></li>";
			},
		}, options.edited.toolbar);

		var HTML = Backbone.Marionette.ItemView.extend(_.extend({
			editTemplate: options.template || "<div><textarea about='{{this}}' cols='"+options.edited.cols+"' rows='"+options.edited.rows+"' placeholder='{{comment}}'>{{"+options.metaKey+"}}</textarea></div>",
			className: "ux_html",
			initialize: function() {
...
DEBUG && console.debug("HTML Hover Off", contained, event, $(event.target), self.$el);
			...
			decideTemplate: function () {
    			...
    			viewTemplate = "<div about='{{this}}' class='ux_html_view'>"+viewTemplate+"</div>";
...
DEBUG && console.debug("HTML TextArea:", this, self.options, $textarea);
                if (!$textarea.length) throw "urn:factcore:ux:oops:missing-textarea";
...
DEBUG && console.debug("HTML wysihtml5", _options, this.wysihtml5, this.editor );
                ...
                $(".btn", this.wysihtml5.toolbar).click(function(event) {
                    var $this = $(this);
                    var method = $this.attr("data-factcore-event") || $this.attr("data-wysihtml5-command");
...
DEBUG && console.debug("HTML onToolbarInsertProperty", value, view, $this, event);
		...
		}, my.factcore.UX.mixin.Common, my.factcore.UX.mixin.Attachable, my.factcore.UX.mixin.Droppable), {
			sample: options.template || "<label class='ux_sample' about='{{this}}'>Example: {{label}}</label>",
			className: "ux_html"
		});
	...
	my.factcore.UX.View["urn:factcore:ux:HTMLPages"] = function(options) {
		options = my.factcore.UX.checkOptions(options);
		...
		var HTML = Backbone.Marionette.ItemView.extend({
			template: "<div class='ux_sample'><label about='{{this}}'>{{label}}</label><div class='ux_toggled ux_preview'>{{{template}}}</div></div>"
		});
//		var HTML = my.factcore.UX.View["urn:factcore:ux:HTML"](options);
		if (!HTML) throw "urn:factcore:ux:oops:missing-widget";

		var Pages = Backbone.Marionette.CompositeView.extend( _.extend({
			itemView: HTML, tagName: "ul", className: my.factcore.UX.stylize("ux_pages nav-list", options),
			template: "<div><label about='{{this}}'>{{label}}</label><ul></ul><div>",
